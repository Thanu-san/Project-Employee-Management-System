...
void Employee::displayDetails(bool showPassword) const {
    int termWidth = 80;
#ifdef _WIN32
    ...
    std::string passwordDisplay = showPassword ? password : "********";
    std::string passwordColor   = showPassword ? RED : GRAY;
    ...
    std::cout << p << CYAN << "|" << RESET << "  " << GRAY << std::left << std::setw(9) << "Password" << RESET << " " << CYAN << ":" << RESET << "  " << passwordColor << std::setw(28) << passwordDisplay << RESET << CYAN << "|" << RESET << "\n";
